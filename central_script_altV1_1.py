...
# ─────────────────────────────────────────────────────────────────────────────
# CLOUD TO LOCAL PASSTHROUGH RELAY 
# ─────────────────────────────────────────────────────────────────────────────

# Cloud Config (Match server IP from test_mqtt.py)
...
def on_cloud_message(client, userdata, msg):
    """
    Callback that catches incoming cloud messages and immediately
    relays them locally to the robot/control topic.
    """
    try:
        command_payload = msg.payload.decode('utf-8').strip()
        print(f"[CLOUD BRIDGE] Received command from cloud: {command_payload}")
        
        # Connect to the local MQTT broker running on the Pi
        import paho.mqtt.client as local_mqtt
        local_client = local_mqtt.Client()
        local_client.connect("192.168.1.145", 1883, 60)
        
        # Pass the message completely untouched to the local robot topic
        local_client.publish("robot/control", command_payload)
        local_client.disconnect()
        print(f"[CLOUD BRIDGE] Successfully relayed locally to 'robot/control'")
        
    except Exception as e:
        print(f"[CLOUD BRIDGE] Error relaying cloud message: {e}")

def cloud_bridge_thread():
    """
    Background worker thread that maintains connection to the cloud broker.
    """
    try:
        # Cross-version compatibility handling for paho-mqtt v1.x and v2.x
        try:
            cloud_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1)
        except AttributeError:
            cloud_client = mqtt.Client()
            
        cloud_client.on_message = on_cloud_message
        
        print(f"[CLOUD BRIDGE] Connecting to cloud broker at {CLOUD_MQTT_SERVER}...")
        cloud_client.connect(CLOUD_MQTT_SERVER, CLOUD_MQTT_PORT, 60)
        
        # Subscribe to cloud commands
        cloud_client.subscribe(CLOUD_TOPIC_CMD)
        print(f"[CLOUD BRIDGE] Subscribed to cloud topic: {CLOUD_TOPIC_CMD}")
        
        cloud_client.loop_forever()
    except Exception as e:
        print(f"[CLOUD BRIDGE] Thread crashed or failed to connect: {e}")
